...
| Väli | Väärtus |
|---|---|
| **Name** | `tiim-36-db-subnet-group-meelis` |
| **Description** | `Database subnet group for team 36 - meelis` |
...
Peaksid nägema kinnitussõnumit: **Successfully created subnet group tiim-36-db-subnet-group-meelis**
...
| Väli | Väärtus |
|---|---|
| **Security group name** | `tiim-36-db-sg-meelis` |
| **Description** | `Security group for MySQL database - meelis` |
| **VPC** | `vpc-02fb02f6b35b49a55 (NaVa)` |
...
| Väli | Väärtus |
|---|---|
| **DB instance identifier** | `tiim-36-wordpress-db-meelis` |
...
| **VPC** | `vpc-02fb02f6b35b49a55 (NaVa)` |
| **DB subnet group** | `tiim-36-db-subnet-group-meelis` |
| **Public access** | **No** |
| **Existing VPC security groups** | Eemalda **default**, lisa **tiim-36-db-sg-meelis** |
...
| Key | Value |
|---|---|
| **Name** | `tiim-36-wordpress-db-meelis` |
...
2. Otsi RDS → Databases → kliki `tiim-36-wordpress-db-meelis`
3. Vali **Connect using** alt
4. Kerib alla **Connectivity & security** sektsiooni
5. Kopeeri **Endpoint** (näiteks: `tiim-36-wordpress-db-meelis.c3uyeww2anku.eu-north-1.rds.amazonaws.com`)
...
8. Lisa endpoint koos pordiga: `<endpoint>:3306` → `tiim-36-wordpress-db-meelis.c3uyeww2anku.eu-north-1.rds.amazonaws.com:3306`
...
2. Kirjelduses (Description) on näidatud: `The secret associated with the primary RDS DB instance: arn:aws:rds:eu-north-1:187833180667:db:tiim-36-wordpress-db-meelis`
...
| **Secret description** | `The secret associated with the primary RDS DB instance: arn:aws:rds:eu-north-1:187833180667:db:tiim-36-wordpress-db-meelis` |
...
| Väli | Väärtus |
|---|---|
| **Security group name** | `tiim-36-alb-sg-meelis` |
| **Description** | `Security group for Application Load Balancer - meelis` |
| **VPC** | `vpc-02fb02f6b35b49a55 (NaVa)` |
...
| Väli | Väärtus |
|---|---|
| **Target group name** | `tiim-36-tg-meelis` |
| **Protocol** | **HTTP** |
| **Port** | **80** |
| **VPC** | `vpc-02fb02f6b35b49a55 (NaVa)` |
...
| Väli | Väärtus |
|---|---|
| **Load balancer name** | `tiim-36-alb-meelis` |
...
| **Protocol** | **HTTP** |
| **Port** | **80** |
| **Routing action** → **Target group** | Vali **tiim-36-tg-meelis** |
...
1. **Details** → **DNS name** - Kopeeri DNS name (näiteks: [tiim-36-alb-meelis-1234567890.eu-north-1.elb.amazonaws.com](http://tiim-36-alb-meelis-1234567890.eu-north-1.elb.amazonaws.com))
...
| Väli | Väärtus |
|---|---|
| **Task definition family** | `tiim-36-wordpress-task-meelis` |
...
| Väli | Väärtus |
|---|---|
| **Cluster name** | `tiim-36-cluster-meelis` |
...
1. Kliki äsjaloodud cluster'i nimel **tiim-36-cluster-meelis**
...
| **Task definition** | Vali **tiim-36-wordpress-task-meelis** (viimane versioon) |
| **Service name** | `tiim-36-service-meelis` |
| **Desired tasks** | **1** |
...
| **Security group** | Vali **Use an existing security group** → `tiim-36-alb-sg-meelis` |
...
| **Load balancer** | Vali **Use an existing load balancer** → `tiim-36-alb-meelis` |
| **Listener** | **Use an existing listener** → **80:HTTP** |
| **Target group** | Vali **Use an existing target group** → `tiim-36-tg-meelis` |
...
1. Kliki loodud service'il **tiim-36-service-meelis**
...
1. Mine **EC2** → **Target Groups** → kliki `tiim-36-tg-meelis`
...
2. Sisesta ALB DNS name (kopeeritud sammust 10.9)
   - Näiteks: `http://tiim-36-alb-meelis-1234567890.eu-north-1.elb.amazonaws.com`
3. Vajuta Enter
...
| Ressurss | Nimi |
|---|---|
| RDS Database | `tiim-36-wordpress-db-meelis` |
| ECR Repository | `tiim_36/wordpress` |
| Target Group | `tiim-36-tg-meelis` |
| Load Balancer | `tiim-36-alb-meelis` |
| ECS Cluster | `tiim-36-cluster-meelis` |
| ECS Service | `tiim-36-service-meelis` |
| Task Definition | `tiim-36-wordpress-task-meelis` |
